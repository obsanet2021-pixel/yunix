...
CREATE TABLE IF NOT EXISTS public.telegram_support_agents (
...
CREATE TABLE IF NOT EXISTS public.support_group_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.user_telegram_sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE IF EXISTS public.telegram_support_agents ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.support_group_config ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.ticket_messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.user_telegram_sessions ENABLE ROW LEVEL SECURITY;

-- RLS policies for telegram_support_agents
DROP POLICY IF EXISTS "CEO can manage support agents" ON public.telegram_support_agents;
CREATE POLICY "CEO can manage support agents" ON public.telegram_support_agents
FOR ALL USING (is_ceo(auth.uid()));

DROP POLICY IF EXISTS "Staff can view support agents" ON public.telegram_support_agents;
...
DROP POLICY IF EXISTS "CEO can manage support group config" ON public.support_group_config;
CREATE POLICY "CEO can manage support group config" ON public.support_group_config
FOR ALL USING (is_ceo(auth.uid()));

DROP POLICY IF EXISTS "Staff can view support group config" ON public.support_group_config;
CREATE POLICY "Staff can view support group config" ON public.support_group_config
FOR SELECT USING (EXISTS (SELECT 1 FROM staff WHERE staff.user_id = auth.uid()));

-- RLS policies for ticket_messages
DROP POLICY IF EXISTS "Staff can manage all ticket messages" ON public.ticket_messages;
...
DROP POLICY IF EXISTS "Service role only for telegram sessions" ON public.user_telegram_sessions;
CREATE POLICY "Service role only for telegram sessions" ON public.user_telegram_sessions
FOR ALL USING (false);

-- Enable realtime for ticket_messages if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    WHERE pub.pubname = 'supabase_realtime'
      AND cls.relname = 'ticket_messages'
